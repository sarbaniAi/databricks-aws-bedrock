...
# already uploaded the pdfs in volume
# %sql
# CREATE EXTERNAL VOLUME IF NOT EXISTS pdf_volume LOCATION '${spark.sql.conf.bucket}';
...
embeddings = deploy_client.predict(endpoint="databricks-gte-large-en", inputs={"input": ["what are the Cancer Diagnosis Benefit mentionewd in the hdfc policy?"]})
pprint(embeddings)
...
deploy_client = get_deploy_client('databricks')
gte_embeddings = deploy_client.predict(endpoint="databricks-gte-large-en", inputs={"input": ["what are the Cancer Diagnosis Benefit mentionewd in the hdfc policy?"]})
...
deploy_client = get_deploy_client('databricks')
titan_embeddings = deploy_client.predict(endpoint=embeddings_model_endpoint_name, inputs={"input": "what are the Cancer Diagnosis Benefit mentionewd in the hdfc policy?"})
...
# MAGIC %md
# MAGIC Notice that a key difference between Titan and BGE is the number of dimensions. In addition, 
# MAGIC there are implementation differences in the API input formats: BGE requires a list vs. Titan requires a string. 
# MAGIC
...
question = "What HDFC Policy says for Limited Pay and Regular Pay Policy for click-2-wealth?"
